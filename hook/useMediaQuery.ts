...
function reducer<T>(state: State<T>, action: Action<T>): State<T> {
  switch (action.type) {
    case "updateMatches":
      return {
        matches: Object.keys(state.mediaQueries).reduce((prev: Matches<any>, key: string) => {
 ...
 */
export function useMediaQueries<T>(queryMap: MediaQueries<T>): MediaQueryMatches<T> {
  const prevQueries = React.useRef<MediaQueries<T>>(queryMap);
  const [state, dispatch] = React.useReducer<React.Reducer<State<T>, Action<T>>, MediaQueries<T>>(reducer, queryMap, init);
