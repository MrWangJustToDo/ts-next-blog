...
const actionReducerMap: StateActionMapType<BlogContentProps[]> = {
  [serverAction.GETDATALOADING(apiName.userHome)]: (state, action) =>
    produce(state, (proxy) => {
      proxy.data = [];
      proxy.error = null;
  ...
  [serverAction.GETDATASUCESS(apiName.userHome)]: (state, action) =>
    produce(state, (proxy) => {
  ...
  [serverAction.GETDATAFAIL(apiName.userHome)]: (state, action) =>
    produce(state, (proxy) => {
      proxy.data = [];
